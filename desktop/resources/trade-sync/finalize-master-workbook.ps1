...
function Resolve-MasterPath([string]$Root, [string]$RequestedMaster) {
  ...
  if ($env:WILYTRADER_MASTER_TRADING_LOG) {
    return [System.IO.Path]::GetFullPath($env:WILYTRADER_MASTER_TRADING_LOG)
  }
...
function Set-FormulaDown($sheet, [string]$column, [int]$firstRow, [int]$lastRow, [string]$formula) {
  ...
  try {
    try {
      ...
      Invoke-ExcelRetry { $topCell.Formula = $formula } | Out-Null
    }
  } catch {
    throw "Failed setting formula in ${column}${firstRow} through ${column}${lastRow}: $formula. $($_.Exception.Message)"
  }
