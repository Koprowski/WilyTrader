...
const TRADE_LOG_HEADER_ALIASES = new Map([
  ...
  ["entry_time_actual", "entry_commentary_time"],
  ["entry_time_inferred", "entry_commentary_time"],
...
function resolveCapturesRoot() {
  ...
  if (process.env.WILYTRADER_CAPTURES_ROOT) {
    return path.resolve(process.env.WILYTRADER_CAPTURES_ROOT);
  }
  ...
  return path.resolve(process.cwd(), "WilyTrader Captures");
}
...
function resolveMasterPath() {
  ...
  if (process.env.WILYTRADER_MASTER_TRADING_LOG) {
    return path.resolve(process.env.WILYTRADER_MASTER_TRADING_LOG);
  }
...
function fillTimeBucketFields(row) {
  ...
  const serial = parseDateSerial(rowDateValue(row));
  const time = parseTimeParts(rowEntryTimeValue(row));
...
function sortRowsForAppend(rows) {
  return [...rows].sort((a, b) => {
    const aKey = sortableDateTimeKey(rowDateValue(a), rowEntryTimeValue(a) || a.exit_time_actual);
    const bKey = sortableDateTimeKey(rowDateValue(b), rowEntryTimeValue(b) || b.exit_time_actual);
...
function renderSyncNicsPrompt(sessionDir, rawRows, evidence) {
  ...
  return `You are reconciling WilyTrader trade rows during master-log sync.
...
async function runGeminiCliForNics(promptText) {
  ...
  const timeoutMs = Number(process.env.WILYTRADER_NICS_SYNC_TIMEOUT_MS ?? process.env.SNIPALOT_NICS_SYNC_TIMEOUT_MS ?? 8 * 60 * 1000);
  const instruction = "Grade the WilyTrader trade rows from stdin. Return only the requested JSON array.";
...
function readGeminiCliSyncConfig() {
  let command = process.env.WILYTRADER_GEMINI_CLI_COMMAND || process.env.SNIPALOT_GEMINI_CLI_COMMAND || "";
  let model = process.env.WILYTRADER_GEMINI_CLI_MODEL || process.env.SNIPALOT_GEMINI_CLI_MODEL || "";
  if (!command || !model) {
...
function clusterDateCode(row, sessionName) {
  const serial = parseDateSerial(firstNonBlank(rowDateValue(row), folderDate(sessionName)));
...
function rowEntryDateTimeMs(row) {
  return rowDateTimeFromTimeValue(row, rowEntryTimeValue(row));
}
...
function rowDateTimeMs(row) {
  return rowDateTimeFromTimeValue(row, firstNonBlank(rowEntryTimeValue(row), row.exit_time_actual));
}
...
function rowDateValue(row) {
  return firstNonBlank(row.entry_date, row.trade_date);
}

function rowEntryTimeValue(row) {
  return firstNonBlank(row.entry_commentary_time, row.entry_time_inferred, row.entry_time_actual, row.video_start_time);
}
...
function corePropsXml() {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:creator>WilyTrader master trading log flow</dc:creator><cp:lastModifiedBy>WilyTrader master trading log flow</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">${now}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${now}</dcterms:modified></cp:coreProperties>`;
}
